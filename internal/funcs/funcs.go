...
func Map() template.FuncMap {
	return template.FuncMap{
		...
		// Trimming functions
		//
		// IMPORTANT: trimPrefix, trimSuffix, trimLeft, trimRight, and trimChars
		// use wrapper functions (not direct stdlib bindings) to reorder arguments
		// for Go template pipeline compatibility. In pipelines, the piped value
		// becomes the LAST argument. Without wrappers, {{ "hello.txt" | trimSuffix ".txt" }}
		// would call strings.TrimSuffix(".txt", "hello.txt") which is wrong.
		// Do NOT replace these with direct strings.TrimPrefix, etc.
		...
		// String manipulation
		//
		// IMPORTANT: contains, hasPrefix, and hasSuffix use wrapper functions
		// (not direct stdlib bindings) to reorder arguments for pipeline
		// compatibility. Without wrappers, {{ "hello" | contains "ell" }}
		// would call strings.Contains("ell", "hello") which is wrong.
		// Do NOT replace these with direct strings.Contains, etc.
		"replace":   replace,
		"replaceN":  replaceN,
		"contains":  contains,
		"hasPrefix": hasPrefix,
		...
		//
		// IMPORTANT: split and splitN use wrapper functions (not direct stdlib
		// bindings) to reorder arguments for pipeline compatibility. Without
		// wrappers, {{ "a,b,c" | split "," }} would call strings.Split(",", "a,b,c")
		// which is wrong. Do NOT replace these with direct strings.Split, etc.
		"split":   split,
		"splitN":  splitN,
		"join":    join,
		"lines":   lines,
...
// Pipeline-compatible wrapper functions for strings package.
//
// Go's text/template pipelines pass the piped value as the LAST argument to
// the function. The stdlib string functions take the string-to-operate-on as
// the FIRST argument (e.g., strings.TrimSuffix(s, suffix)). This means
// {{ "hello.txt" | trimSuffix ".txt" }} would incorrectly call
// strings.TrimSuffix(".txt", "hello.txt") without these wrappers.
//
// These wrappers reorder arguments so the string-to-operate-on is LAST,
// matching the pattern used by replace, replaceN, and join.
//
// DO NOT replace these wrappers with direct stdlib bindings — doing so will
// silently break all pipeline usage of these functions in user templates.
...
func trimLeft(cutset, s string) string {
	return strings.TrimLeft(s, cutset)
}

func trimRight(cutset, s string) string {
	return strings.TrimRight(s, cutset)
}

func trimChars(cutset, s string) string {
...
func splitN(sep string, n int, s string) []string {
